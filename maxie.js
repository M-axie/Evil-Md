...
const botname = process.env.BOTNAME || 'EVIL MD';
const antibot = process.env.ANTIBOT || 'FALSE';
    ...
    const author = process.env.STICKER_AUTHOR ||'Its me';
    const packname = process.env.STICKER_PACKNAME || 'Ma_Xie';
const dev = process.env.DEV || '254716029292'
...
const admin = process.env.ADMIN_MSG || 'Uff!\n Admins are only allowed to use this command dawg.';
    const group = process.env.GROUP_ONLY_MSG || 'Add me to your group dawg for you to use my group commands.';
    const botAdmin = process.env.BOT_ADMIN_MSG || 'Ain't an admin lol🙄'
    const NotOwner = process.env.NOT_OWNER_MSG || 'Only owner can command me lol😏';
...
function _0x2de4() {
    const _0xca7309 = [
        ...
        'catch',
        'e\x20text.\x20Th',
        '\x20of\x20its developer ᳃𖤇⍣⃝ 𝞢𝝯𝞘𝙇 𝙎𝞗𝞜👿༒꧂💀',
        'is\x20AI\x20work',
        '1832596QZCVhJ',
        '\x20and\x20Googl',
        'AgaVJ',
...
break;
 ...
 m.reply('My Owner is now an Admin🤠 ✞'); 
          }
          break;

case 'restart':  
  if (!Owner) throw `Owner Me dawg` 
  reply(`Restarting. . .`)  
  await sleep(3000)  
  process.exit()  
  break;
...
try {
 ...
 if (users  == client.decodeJid(client.user.id)) throw 'You cannot kick my bot owner lol🙄';
 if (users == Owner) { m.reply('Its owner number')}; 
                 await client.groupParticipantsUpdate(m.chat, users, 'remove'); 
await m.reply('**Kicked mercilessly without fear by EVIL-MD🫨**'); 
} catch (errr) { 
 await reply("Dude!\n You just know clearly I am not an admin.....Use your common senss bratt!!)}
...
`👋🏻 Hi *${pushname}*,You can deploy 𝐃𝐑𝐄𝐗 𝐁𝐎𝐓 using the GitHub link below🗿\n\nFork and give us a star✨.\n\n https://github.com/drexmose/drex-ai\n\nEnjoy and have fun with the bot🗿...\n\nUse the link below to pair 𝐃𝐑𝐄𝐗_𝐀𝐈 without scanning any qr code\n\n [https://replit.com/@darkintent120/Pairing-drex9]\n\nMade on Earth by 𝕯⃟𝗮𝗿𝗸_𝗜𝗻𝘁𝗲𝗻𝘁⃟ꦿ⸼` });
 ...
 break; 
         ...
         if (!isBotAdmin) throw botAdmin; 
         if (!isAdmin) throw admin; 
 if (!m.quoted) throw `Tag that successful person Sir!`; 
                 let users = m.mentionedJid[0] ? m.mentionedJid : m.quoted ? [m.quoted.sender] : [text.replace(/[^0-9]/g, '')+'@s.whatsapp.net']; 
  
                 await client.groupParticipantsUpdate(m.chat, users, 'promote'); 
 m.reply('The group is happy to crown you as a new admin🤗'); 
         } 
 break; 
 case "demote": { 
                 if (!m.isGroup) throw group; 
         if (!isBotAdmin) throw botAdmin; 
         if (!isAdmin) throw admin; 
 if (!m.quoted) throw `Tag that creepy asshole Sir!`; 
                 let users = m.mentionedJid[0] ? m.mentionedJid : m.quoted ? [m.quoted.sender] : [text.replace(/[^0-9]/g, '')+'@s.whatsapp.net']; 
  
                 await client.groupParticipantsUpdate(m.chat, users, 'demote'); 
 m.reply('The group is happy to snatch away your throne bratt!!'); 
         } 
 break;
 case "disp7": { 
                 if (!m.isGroup) throw group; 
 ...
 m.reply('Dissapearing messages successfully turned off!'); 
    ...
    reply('My Owner has successfully updated the group icon😌'); 
    } 
    break;
          case "revoke": 
 case "newlink": 
 ...
 client.sendText(m.chat, `Sent you the new group link in your inbox!`, m); 
  ...
  if (!isBotAdmin) throw botAdmin; 
  if (!isAdmin) throw admin; 
    if (!m.quoted) throw `You have not tagged the message to be deleted`; 
 ...
 break;
          case "leave": { 
                 if (!isAdmin) throw admin; 
 await client.sendText(m.chat,  'Whooaa! Finally my owner is exiting your useless group. . .'); 
 ...
 case "tagall": { 
                 ...
                 if (!isBotAdmin) throw botAdmin; 
                 if (!isAdmin) throw admin; 
 let teks = `Heyy!!\n : So sorry you are tagged by EVIL MD
